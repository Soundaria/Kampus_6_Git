...
namespace CaseStudyKampusLearnAPI.Controllers
{
	...
	public class AdminController : ControllerBase
	{
		...
		public IActionResult AddAdmin([FromBody] Admin admin)
		{
			try
			{
				List<User> userLists = repo.User.ToList();
				var duplicateUser = userLists.Find(x => x.Email == admin.Email);
				if (duplicateUser == null)
				{
					...
					repo.User.Add(user);
					repo.SaveChanges();
					List<User> userList = repo.User.ToList();
					var adminUser = userList.Find(x => x.Email == admin.Email);
					admin.AdminId = adminUser.UserId;
		...
		public IActionResult UpdateAdmin([FromBody] Admin adminobj)
		{
			try
			{
				...
				Admin admin = repo.Admin.Find(adminId);
				User user = repo.User.Find(adminId);
				if (admin != null)
				{
					...
					user.Email = adminobj.Email;
					user.Password = adminobj.Password;
					repo.SaveChanges();
		...
		public IActionResult AddTrainer([FromBody] Trainer trainer)
		{
			...
			{
				List<User> userLists = repo.User.ToList();
				var duplicateUser = userLists.Find(x => x.Email == trainer.Email);
				if (duplicateUser == null)
				{
					User user = new User();
					...
					repo.SaveChanges();
					List<User> userList = repo.User.ToList();
					var TrainerUser = userList.Find(x => x.Email == trainer.Email);
					int adminId = Convert.ToInt32(HttpContext.User.FindFirstValue("adminId"));
					trainer.TrainerId = TrainerUser.UserId;

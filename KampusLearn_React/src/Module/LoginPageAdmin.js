...
function LoginAdmin(){
    ...
    const Validation=(e)=>{
        ...
        axios.post('https://localhost:7211/api/Admin/ValidatingAdmin',{
            email,
            password
        }).then((response)=>{
    ...
    //     axios.get(`https://localhost:7211/api/Admin/ValidatingAdminGoogle/${email}`
    //         ).then((response)=>{
    //         console.log(response.data);
    //         localStorage.setItem("Authtoken",JSON.stringify(response.data.jwt));
    //         localStorage.setItem("Name",JSON.stringify(response.data.id.name));
        ...
        <Center>
            ...
            <GoogleLogin
             onSuccess={credentialResponse => {
              ...
              console.log(user);
              axios.get(`https://localhost:7211/api/Admin/ValidatingAdminGoogle/${user.email}`
                      ).then((response)=>{
                      console.log(response.data);
                      localStorage.setItem("Authtoken",JSON.stringify(response.data.jwt));
                      localStorage.setItem("Name",JSON.stringify(response.data.id.name));
